...
res_shc_r <- shc(patient_matrix, matmet=mfun, linkage="ward.D2", n_sim = 1000)
res_shc_r$hc_dat$labels <- rownames(patient_matrix)

png(file="~/Vascular_Disease/plots/hc/hc_r.png", width =465, height = 225, units = "mm", res=300)
plot(res_shc_r,alpha=0.05,ci_emp=T,use_labs = TRUE)
dev.off()
...
u <- 4 # 1 to 11 because of non 0-based language: If we want, for example, theta to be 0, we set u=1
val <- 29  #lambda
...
plot(res_hclust)
res_shc_h <- shc(t(patient_matrix3), matmet=mfun, linkage="ward.D2", n_sim = 1000,alpha = 0.05)
res_shc_h$hc_dat$labels <- rownames(t(patient_matrix3))

recap <- sigclust2::shcutree(res_shc_h,alpha = .05)
names(recap) <- res_shc_h$hc_dat$labels

png(file="~/Vascular_Disease/plots/hc/hc_rec.png", width =465, height = 225, units = "mm", res=300)
plot(res_shc_h,alpha=0.05,ci_emp=T,use_labs = TRUE)
dev.off()
...
library(gprofiler2)
pathways_res <- lapply(genes_interesantes, function(x) gost(x,'hsapiens', correction_method ="bonferroni", domain_scope="custom", custom_bg=all_genes_symbol))
...
#Venn diagram
png(file="~/Vascular_Disease/plots/hc/venn.png", width =465, height = 225, units = "mm", res=300)
